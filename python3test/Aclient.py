...
class LoginFrame(wx.Frame):
    ...
    def __init__(self, parent, id, title, size):
        ...
        self.loginButton = wx.Button(self, label = 'Login', pos = (80, 145), size = (120, 30))
        self.loginButton.Bind(wx.EVT_BUTTON, self.login)
        self.serverAddress.SetForegroundColour('black')
        self.serverAddress.SetBackgroundColour('yellow')
        self.Show()
    ...
    def login(self,event):
        ...
        try:
            ...
            if response != 'Connect Success':
                self.showDialog('Error', 'Connect Fail!', (180, 30))
                return
            con.write('login ' + str(self.userName.GetLineText(0)) + '\n')
            response = con.read_some()
            if response == 'UserName Empty':
                self.showDialog('Error', 'UserName Empty!', (180, 30))
            elif response == 'UserName Exist':
                self.showDialog('Error', 'UserName Exist!', (180, 30))
            else:
                self.Close()
                ChatFrame(None, -2, title = 'ShiYanLou Chat Client', size = (500, 350))
        except Exception:
            self.showDialog('Error','Connect Fail!', (195, 80))
